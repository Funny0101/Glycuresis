<template>
  ...
  <div v-if="showSelectArea" class="search">
    <div class="food-container">
      <van-nav-bar title="推荐食谱" left-arrow left-text="返回" @click-left="() => { this.$router.go(-1); }"
  ...
  <div v-else>
    <!-- 横向滑动的tab栏(季节) -->
    <van-tabs v-model:active="activeSeason" sticky @change="onChangeTabSeason" swipeable color="green"
      class="tab-season">
      <van-tab v-for="(season, index) in seasons" :key="index" :title="season.name">
      </van-tab>
    </van-tabs>
    <!-- 横向滑动的tab栏(数字) -->
    <van-tabs v-model:active="activeNumber" sticky @change="onChangeTabNumber" swipeable color="green"
      class="tab-number">
    ...
    <div class="recipe" v-if="!isLoading">
      ...
      <div class="every_meal" style="background-color: #d7f4e3;">
        ...
        <div class="card">
          <ul>
            <li v-for="(item, index) in recipe.breakfast" :key="index" style="font-size: 16px;">{{ item }}</li>
          </ul>
        </div>
      </div>
      <!-- 午餐卡片 -->
      <div class="every_meal" style="background-color: #f2c4ab;">
        ...
        <div class="card">
          <ul>
            <li v-for="(item, index) in recipe.lunch" :key="index" style="font-size: 16px;">{{ item }}</li>
          </ul>
        </div>
      </div>
      <!-- 晚餐卡片 -->
      <div class="every_meal" style="background-color: #bde6f8;">
        ...
        <div class="card">
          <ul>
            <li v-for="(item, index) in recipe.dinner" :key="index" style="font-size: 16px;">{{ item }}</li>
          </ul>
        </div>
      </div>
      <!-- 油盐卡片 -->
